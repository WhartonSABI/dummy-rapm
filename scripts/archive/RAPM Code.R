library(dplyr)
# Project path bootstrap (works when launched from repo root or scripts/)
...
if (file.exists(file.path(repo_root, "scripts")) && getwd() != repo_root) {
  setwd(repo_root)
}
...
play_by_play_data_small <- play_by_play_data_small %>%
  mutate(
    possession_end = case_when(
      ...
      # Last free throw of a sequence (only ends possession if made)
...
ggsave(file.path("plots", "coefficient_comparison_plot.png"), coefficient_comparison,
...
ggsave(file.path("plots", "coefficient_distribution_overlay.png"), distribution_overlay,
       width = 10, height = 8, dpi = 300)
...
# Top and bottom 10 for no_dummy
