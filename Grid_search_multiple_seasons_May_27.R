...
for (season in seasons_to_run) {
  ...
  for (min_minutes in minutes_thresholds) {
    ...
    X_no_dummy <- do.call(
      ...
      lapply(
        ...
        function(i) {
          ...
          as.integer(
            all_players_no_dummy %in% home
          ) -
            as.integer(
              all_players_no_dummy %in% away
            )
        }
      )
    )
    ...
    X_no_dummy_sparse <- Matrix(
      X_no_dummy,
      sparse = TRUE
    )
    
    X_no_dummy_train <- X_no_dummy_sparse[
      train_idx,
    ]
    
    X_no_dummy_test <- X_no_dummy_sparse[
      test_idx,
    ]
...
ggplot(
  ...
  geom_point(size = 3) +
  labs(
    title = "Average No-Dummy RMSE by Minutes Threshold",
    x = "Minimum Average Minutes Threshold",
    y = "Average RMSE"
  ) +
  theme_minimal()
...
model_results <- bind_rows(
  ...
  final_grid_results %>%
    transmute(
      ...
      min_minutes,
      model_type = "no_dummy",
      model_name = paste0(min_minutes, "_no_dummy"),
      RMSE = no_dummy_RMSE,
      R2   = no_dummy_R2
    )
)
