...
RAPM reduces mean game-margin RMSE from \MeanNoDummyRMSE{} to
...
seasons combined into a common reference category. Here the threshold is mean
minutes per appearance, calculated only from the relevant training period.
This measure reflects a player's role when active and avoids classifying a
regular rotation player as low-minute solely because he missed games. The
threshold is selected for prediction; it is not a ranking of player quality.

\section{Data Construction and Validation}

We use publicly accessible ESPN play-by-play and player box-score records
distributed through the sportsdataverse project \citep{Gilani2026}. Official
box-score starter flags initialize each five-player lineup; recorded
substitutions update the lineup thereafter. A game is excluded if either team
does not have exactly five unique recorded starters. Across the primary
sample, \StarterExcludedGames{} games are excluded for this reason, including
...
possession. A zero-exposure scoring segment---for example, free throws
separated by a substitution---is merged with the next positive-exposure stint,
or with the preceding stint if it occurs at the end of the game. Thus no
scored point is assigned an infinite per-possession response. The reconstructed
stint margins must sum exactly to the official final margin in every included
game. Within each game, period labels must form a contiguous sequence beginning
with period 1. Records after the first gap in that sequence are excluded,
while valid multi-overtime sequences are retained.
...
full-game prediction target. The analysis code is open source; the underlying
ESPN records are publicly accessible.
...
\]
Predictive performance is reported with game-margin RMSE and out-of-sample
\[
R^2_{\mathrm{oos}}
=
...
\begin{tabular}{lccc}
\toprule
Model & Mean RMSE & Mean $R^2$ & Lower-RMSE seasons \\
\midrule
...
Dummy RAPM reduces mean game-margin RMSE from \MeanNoDummyRMSE{} to
...
Mean out-of-sample $R^2$ rises from \MeanNoDummyRSquared{} to
...
$t(\PairedTDegreesFreedom)=\PairedTStatistic$ and $p=\PairedTPValue$; the
Wilcoxon signed-rank test gives $W=\WilcoxonStatistic$ and
...
Filtering low-minute players without adding count indicators does not improve
RAPM. Relative to the unfiltered benchmark, the selected filtered model is
worse by
...
The selected ratio, $\rho=\SelectedPenaltyRatio$, shrinks the count
coefficients more heavily than the player coefficients. Nearby ratios perform
nearly identically, so the precise value of \SelectedPenaltyRatio{} is less
important than the broader result: the data favor substantial shrinkage of the
count coefficients rather than their exclusion.
...
so they contribute little to the substantive pattern. Finally, adjacent
seasons share players, coaches, and league conditions, so the
\NumberSeasons{} season-level comparisons should not be viewed as fully
