# Grid Search Minimum Average Training Set Minutes
...
start_five <- all_player_events %>%
  ...
  slice_head(n = 5) %>%
  summarise(
    starting_five = list(trimws(as.character(player_id))),
    .groups = "drop"
  )
...
for (grp in lineup_changes) {
  ...
  for (i in seq_len(n)) {
    ...
    if (!is.null(grp$starting_five[[i]]) && length(grp$starting_five[[i]]) == 5) {
      updated_lineups[[i]] <- trimws(as.character(grp$starting_five[[i]]))
      
      # Case 2: Otherwise, carry over previous lineup
    } else if (i > 1 && !is.null(updated_lineups[[i - 1]])) {
      updated_lineups[[i]] <- trimws(as.character(updated_lineups[[i - 1]]))
    } else {
    ...
    if (grp$type_text[i] == "Substitution" &&
      ...
      out_player  <- trimws(as.character(grp$athlete_id_2[i]))
      in_player   <- trimws(as.character(grp$athlete_id_1[i]))
      lineup      <- trimws(as.character(updated_lineups[[i]]))
      ...
      if (in_already && out_present) {
        ...
        # In-player already there, out-player not found — skip
...
# Roll 0-possession scoring into the next stint with possessions
base_stints <- base_stints_raw %>%
  group_by(game_id) %>%
  mutate(
    home_net_points = home_net_points + 
      if_else(lag(n_possessions, default = 1) == 0, lag(home_net_points, default = 0), 0L)
  ) %>%
