...
# Qualified players: more than 1200 total minutes over the whole season
...
y <- 100 * stints$home_net_points / stints$n_possessions
...
test_results_dummy <- data.frame(
  ...
  actual_y = y_test,
  predicted_y = as.vector(
    predict(dummy_ridge_model, newx = X_test, s = "lambda.min")
  ),
  possessions = w_test
) %>%
  group_by(game_id) %>%
  summarise(
    actual_margin =
      sum(actual_y * possessions / 100),
    
    predicted_margin =
      sum(predicted_y * possessions / 100),
...
test_results_no_dummy <- data.frame(
  ...
  predicted_y = as.vector(
    predict(no_dummy_ridge_model, newx = X_no_dummy_test, s = "lambda.min")
  ),
  possessions = w_test
) %>%
  group_by(game_id) %>%
...
perf <- data.frame(
  ...
  Margin_RMSE = c(
    sqrt(mean(
      (test_results_dummy$actual_margin -
         test_results_dummy$predicted_margin)^2
    )),
    sqrt(mean(
      (test_results_no_dummy$actual_margin -
         test_results_no_dummy$predicted_margin)^2
    ))
  ),
  ...
  Margin_R2 = c(
    r_squared(
      test_results_dummy$actual_margin,
      test_results_dummy$predicted_margin
    ),
    r_squared(
      test_results_no_dummy$actual_margin,
      test_results_no_dummy$predicted_margin
    )
  )
)
...
sd(test_results_dummy$actual_margin) # 12.59 rather than 12.58 (very close)
dummy_ridge_model$lambda.min
no_dummy_ridge_model$lambda.min


range(dummy_ridge_model$lambda)
